...
| `feishu_send` | `chat_id`, `text` | 主动发送飞书消息 |

---

## 飞书机器人

通过 WebSocket 长连接实现飞书消息双向通信，无需公网服务器。

- 支持私聊（DM）和群聊消息
- FNV-1a 哈希环形缓冲区消息去重（64 条）
- AI 处理时自动添加 Reaction 表情，完成后移除
- 处理 LLM 请求时暂停 WebSocket 以释放内存
- AI 可通过 `feishu_send` 工具主动发消息
...
5. 发布应用版本
6. 将 `App ID` 和 `App Secret` 填入 `user_config.ini` 或通过串口设置
...
- 飞书 / 搜索 / Cron / 心跳等服务仅在配置了密钥时初始化
- LLM 请求前释放 JsonDocument 以腾出响应缓冲区
- 处理 LLM 请求时暂停飞书 WebSocket 释放内存
