...
M5Stack Cardputer 上的全功能 AI 助手 — 本地交互 × 云端智能 × 微信互联。
...
顶部状态栏显示：`[Alt]back [Tab/Ctrl]scroll` | `[Fn]voice`
...
首页按 `Ctrl` 进入（从底部向上滑入），按 `Tab` 返回（向下滑出）。

- **未配对**：显示配对二维码，微信扫码即可完成连接
- **已配对**：显示在线状态、Cron 任务数、心跳倒计时、API Host、IP 地址
- 按 `Enter` 可重新发起配对
...
| `cron_add` | `name`, `message`, `schedule_type`, ... | 添加定时/周期任务（通道自动继承） |
| `cron_list` | — | 列出所有定时任务 |
...
基于微信 iLink Bot 协议（`@tencent-weixin/openclaw-weixin`），HTTP 长轮询收发消息。

- 设备端扫码配对，无需服务器
- 私聊双向通信，FNV-1a 哈希消息去重
- AI 处理时自动暂停轮询释放 TLS 内存，完成后恢复
- Cron 通知自动路由：微信设的提醒发微信，本地设的只在屏幕显示
- AI 可通过 `wechat_send` 工具主动推送消息

### 配对方法

**方法一：设备扫码（推荐）**

1. 在首页按 `Ctrl` 进入微信配对界面
2. 设备自动向 `ilinkai.weixin.qq.com` 请求二维码
3. 用微信扫描屏幕上的二维码，在手机上确认授权
4. 配对成功后自动保存凭证，界面切换为状态视图
5. 重启后无需重新配对

**方法二：手动配置**

1. 在 PC 端运行 `npx -y @tencent-weixin/openclaw-weixin-cli@latest install`
2. 通过串口 `set_wechat <token> <host>` 或写入 `user_config.ini`

### 微信状态页

在首页按 `Ctrl` 进入，按 `Tab` 返回。

| 状态 | 显示内容 |
|------|---------|
| 未配对 | 配对二维码 + 扫码提示 |
| 已配对 | 在线状态、Cron 数量、心跳倒计时、API Host、设备 IP |
...
| 检查间隔 | 60 秒 |
| 持久化 | `/cron.json`，重启不丢失 |
| 通道路由 | 自动继承请求来源：微信设的提醒发微信，本地设的在屏幕显示 |
| 触发方式 | 通知直接显示/转发，不经过 LLM（避免无限循环） |

AI 可通过 `cron_add` / `cron_list` / `cron_remove` 工具自主管理定时任务。通道和目标 ID 由系统自动设置，无需 LLM 指定。
...
┌──────────────────────────────────────────────────────────┐
│                     main.cpp (loop)                       │
│  SETUP → COMPANION → CHAT → WECHAT_STATUS                │
│          像素宠物     聊天    微信配对/状态                  │
└────────────────────────┬─────────────────────────────────┘
                         │
         ┌───────────────┼───────────────┐
...
│  天气 + 日落  │  │  聊天界面   │  │  ReAct + Tool Call │
└──────┬───────┘  └─────┬──────┘  └─────────┬──────────┘
       │                │                    │
       ▼                ▼                    ▼
┌──────────────┐  ┌────────────┐  ┌────────────────────┐
│  Weather     │  │ SessionMgr │  │    MessageBus      │
│  Client      │  │ MemoryStore│  │  ┌─ WechatBot      │
└──────────────┘  └────────────┘  │  ├─ CronService    │
                                  │  ├─ Heartbeat      │
                                  │  └─ SkillLoader    │
                                  └────────────────────┘
